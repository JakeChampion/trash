...
install() {
	...
	*)
	...
	success "Downloading trash..."

	## Downloading the binaries
	ensure rm -rf "trash-$_target"
	curl -s https://api.github.com/repos/JakeChampion/trash/releases/latest | grep "browser_download_url" | cut -d '"' -f 4 | grep "$_target" | xargs -n 1 curl -LJO

	ensure mv "trash-$_target" "trash_bin"

	ensure sudo mv trash_bin "$_install_path/trash"
	ensure chmod +x "$_install_path/trash"

	success "trash is installed!"
...
cargo_build() {
	...
	RUSTFLAGS="-C target-cpu=native" ensure cargo install trash
}
